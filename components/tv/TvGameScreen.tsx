 ...
 * TvCanvas aplica rotate(-90deg) — diseñamos aquí en portrait sin rotación interna.
 */
function VerticalGame({ gameUrl, messages, msgIndex, imageUrl, textColor }: any) {
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', width: 800, height: 800, background: 'radial-gradient(circle, rgba(255,255,255,0.12) 0%, transparent 70%)', top: '-300px', left: '-200px' }} />
        <div style={{ position: 'absolute', width: 600, height: 600, background: 'radial-gradient(circle, rgba(255,255,255,0.08) 0%, transparent 70%)', bottom: '-200px', right: '-150px' }} />
      </div>
      ...
      {/* Logo + badge + mensaje */}
      <div style={{ position: 'relative', zIndex: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 80, paddingBottom: 20 }}>
        <img src="/logo.png" alt="Alto Drugstore" style={{ height: 220, objectFit: 'contain' }} />
        <div style={{ marginTop: 20, background: 'linear-gradient(135deg, #f6d365, #fda085)', borderRadius: 50, padding: '14px 40px', fontSize: 26, fontWeight: 800, color: '#1a1a2e', letterSpacing: 2, textTransform: 'uppercase', fontFamily: 'Arial, sans-serif' }}>
          🏆 Juego de Premios
        </div>
        <div style={{ marginTop: 24, padding: '0 60px', fontSize: 58, fontWeight: 900, color: textColor, fontFamily: 'Arial Black, Arial, sans-serif', lineHeight: 1.15, textAlign: 'center', textShadow: '0 2px 8px rgba(0,0,0,0.25)' }}>
          {messages[msgIndex]}
        </div>
      </div>

      {/* QR */}
      <div style={{ position: 'relative', zIndex: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, marginTop: 40 }}>
        <div style={{ background: 'white', padding: 20, borderRadius: 24, boxShadow: '0 0 60px rgba(246,211,101,0.4)' }}>
          <QRCodeSVG value={gameUrl} size={240} />
        </div>
        <p style={{ color: textColor, fontSize: 24, margin: 0, fontFamily: 'Arial, sans-serif', fontWeight: 700, textAlign: 'center', textShadow: '0 1px 4px rgba(0,0,0,0.3)' }}>
          Escaneá con tu celular · Gratis
        </p>
      </div>
    </div>
